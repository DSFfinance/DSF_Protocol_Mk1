...
contract CurveConvexStrat_MIM_3CRV is CurveConvexExtraStratBaseMk2 {
    ...
    function depositPool(uint256[3] memory amounts) internal override returns (uint256 poolLPs) {
        ...
        // 3pool slippage
        uint256 expected3Lp = pool3.calc_token_amount(amounts, true);
        uint256 minMint3Pool =
            (expected3Lp * minDepositAmount) / DEPOSIT_DENOMINATOR; // ≤ 99,75 %
        pool3.add_liquidity(amounts, minMint3Pool);
        ...
        uint256 expectedMetaLp = pool.calc_token_amount(amounts2, true);
        uint256 minMintMeta = (expectedMetaLp * minDepositAmount) / DEPOSIT_DENOMINATOR;
        poolLPs = pool.add_liquidity(amounts2, minMintMeta);
    ...
    function sellToken() public {
        ...
        if (sellBal > 0) {
            token.forceApprove(address(pool), sellBal);
        ...
        returns (uint256 depositEfficiency1e18, uint256 roundTripEfficiency1e18)
    {
        ...
        uint256[3] memory a3;
        a3[tokenIndex] = amount;

        uint256 expected3Lp = pool3.calc_token_amount(a3, true);
        uint256[2] memory a2;

        a2[CURVE_3POOL_LP_TOKEN_ID] = expected3Lp;

        uint256 expectedMetaLp = pool.calc_token_amount(a2, true);
        uint256 lpPrice = pool.get_virtual_price(); // 1e18
        uint256 depositValueUsd1e18 = (expectedMetaLp * lpPrice) / CURVE_PRICE_DENOMINATOR;
        ...
        uint256 tokenOut = pool3.calc_withdraw_one_coin(pool3Lps, int128(tokenIndex));
