...
/**
 * @title  CurveConvexStrat_crvUSD_USDC
 * @author Andrei Averin — DSF.Finance
 * @notice Convex strategy for Curve Factory Plain Pool (crvUSD/USDC), Convex PID=182
 *
 * @dev High-level:
 ...
 *
 * Assumptions:
 * - Curve pool coin order MUST be: 0 = USDC, 1 = crvUSD
 ...
 */
contract CurveConvexStrat_crvUSD_USDC is CurveConvexExtraStratBaseMk2 {
    using SafeERC20 for IERC20Metadata;

    /**
     * @notice Curve pool USDC coin index (int128) for 2-coin pool
     * @dev    Expected pool coins: 0 = USDC, 1 = crvUSD
     */
    int128 public constant CURVE_USDC_COIN_ID_INT = 0;

    /// @notice Curve pool interface for crvUSD/USDC pool (2-coin)
     ...
     * @param  poolAddr         Curve pool address (crvUSD/USDC factory pool)
     * @param  poolLPAddr       Curve LP token address
     * @param  rewardsAddr      Convex rewards contract for the PID
     * @param  poolPID          Convex PID (182 for crvUSD/USDC)
    ...
    function depositPool(uint256[3] memory amounts) internal override returns (uint256 poolLPs) {
        ...
        // Curve pool deposit: [USDC, crvUSD]
    ...
    function withdrawAllSpecific() internal override {
        // Remove everything as USDC (simplest, avoids needing crvUSD swaps here)
