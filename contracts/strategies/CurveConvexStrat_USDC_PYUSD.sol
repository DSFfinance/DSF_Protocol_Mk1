...
/**
 * @title  CurveConvexStrat_USDC_PYUSD
 * @author Andrei Averin — DSF.Finance
 * @notice Convex strategy for Curve Factory Plain Pool (USDC/PYUSD), Convex PID=270
 *
 * @dev High-level:
 * - DSF-facing API uses 3 stable tokens in DSF order: DAI(0), USDC(1), USDT(2)
 * - Internally all Curve interactions are USDC-only:
 *   - Deposit: swap DAI/USDT -> USDC via `_config.router`, then `add_liquidity([0,USDC], minMint)`, then stake LP into Convex
 *   - Withdraw: `remove_liquidity_one_coin(..., USDC, ...)` then optionally swap USDC -> requested token(s) via router
 *
 * Assumptions:
 * - Curve pool coin order MUST be: 0 = PYUSD, 1 = USDC
 ...
 */
contract CurveConvexStrat_USDC_PYUSD is CurveConvexExtraStratBaseMk3_NG {
    using SafeERC20 for IERC20Metadata;

    /**
     * @notice Curve pool USDC coin index (int128) for 2-coin pool
     * @dev    Expected pool coins: 0 = PYUSD, 1 = USDC
     */
    int128 public constant CURVE_USDC_COIN_ID_INT    = 1;

    uint256 public constant CURVE_PYUSD_COIN_ID_UINT = 0;
    uint256 public constant CURVE_USDC_COIN_ID_UINT  = 1;

    /// @notice Curve pool interface for USDC/PYUSD pool (2-coin)
     ...
     * @param  poolAddr         Curve pool address (PYUSD/USDC factory pool)
     * @param  poolLPAddr       Curve LP token address
     * @param  rewardsAddr      Convex rewards contract for the PID
     * @param  poolPID          Convex PID (270 for PYUSD/USDC)
    ...
    function depositPool(uint256[3] memory amounts) internal override returns (uint256 poolLPs) {
        ...
        // Curve pool deposit: [PYUSD, USDC]
        uint256[] memory amounts2 = new uint256[](2);
        amounts2[CURVE_PYUSD_COIN_ID_UINT] = 0;             // PYUSD
        amounts2[CURVE_USDC_COIN_ID_UINT]  = usdcToDeposit; // USDC
        ...
        returns (uint256 sharesAmount)
    {
        ...
        a2[CURVE_PYUSD_COIN_ID_UINT] = 0;
        a2[CURVE_USDC_COIN_ID_UINT]  = usdcApprox;
    ...
    function withdrawAllSpecific() internal override {
        // Remove everything as USDC (simplest, avoids needing PYUSD swaps here)
        ...
        returns (uint256 depositEfficiency1e18, uint256 roundTripEfficiency1e18)
    {
        ...
        uint256[] memory a2 = new uint256[](2);
        a2[CURVE_PYUSD_COIN_ID_UINT] = 0;
        a2[CURVE_USDC_COIN_ID_UINT]  = usdcIn;
